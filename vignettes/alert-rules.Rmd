...
The `expression` is evaluated per time series, and must return either `TRUE` or `FALSE`. A return value of `TRUE` means
...
`expression`, the dataframe is grouped by the `item_col` and ordered by the
`timepoint_col`.
