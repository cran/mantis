test_that("rmd_fig_height() returns suitable height for heatmap plots", {
  ...
  inputspec_notab <- inputspec(
    timepoint_col = "timepoint",
    item_cols = "item",
    value_col = "value"
  )

  inputspec_tab <- inputspec(
    timepoint_col = "timepoint",
  ...
  expect_equal(
    rmd_fig_height(
      df = quick_df(items = 1:10, tabs = c(rep("a", 3), rep("b", 7))),
      inputspec = inputspec_tab,
      outputspec = outputspec_static_heatmap()
    ),
    0.6 * 7
  ...
  expect_equal(
    rmd_fig_height(
      df = quick_df(items = c(1:3, 2:8), tabs = c(rep("a", 3), rep("b", 7))),
      inputspec = inputspec_tab,
      outputspec = outputspec_static_heatmap()
    ),
    0.6 * 7
